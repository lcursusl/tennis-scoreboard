  ...
  <div class="container">
    ...
    <div class="pagination">
      <c:if test="${currentPage > 1}">
        <a class="prev" href="?page=${currentPage - 1}&filter_by_player_name=${param.filter_by_player_name}"> < </a>
      </c:if>
      ...
      <c:forEach begin="1" end="${totalPages}" var="page">
        <c:choose>
          ...
          <c:otherwise>
            <a class="num-page" href="?page=${page}&filter_by_player_name=${param.filter_by_player_name}">${page}</a>
          </c:otherwise>
        </c:choose>
      </c:forEach>

      <c:if test="${currentPage < totalPages}">
        <a class="next" href="?page=${currentPage + 1}&filter_by_player_name=${param.filter_by_player_name}"> > </a>
      </c:if>
    </div>
  </div>
</main>
